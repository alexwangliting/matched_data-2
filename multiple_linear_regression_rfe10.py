...
def run_regression(df: pd.DataFrame, y: str, X: List[str], model_name: str) -> None:
    ...
    print(f"\n=== Linear Regression Results: {model_name} (RFE Top 10) ===")
    print(model.summary())
    # Residual plot
    ...
    plt.title(f"Residual Plot: {model_name} (RFE Top 10)")
    plt.tight_layout()
    out_path = f"{PLOT_DIR}residuals_{model_name.replace(' ', '_').lower()}_rfe10.png"
...
def main() -> None:
    """Run and compare linear regression models for both outcomes using RFE top 10 features."""
    df = load_data(INPUT_PATH)
    for y, model_name in OUTCOMES:
        run_regression(df, y, RFE10_FEATURES, model_name)
